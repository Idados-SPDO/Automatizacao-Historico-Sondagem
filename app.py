...
if arquivo_atualizacao is not None:
    if tipo_sondagem != 'SEBRAE':
        ...
        df_mailing_historico = pd.read_excel(f'W:/Otimização e Automatização/5. Projetos/22. Produção dos Pesquisadores/1. Dados/Histórico/historico_{tipo_sondagem}.xlsx', sheet_name = "Mailing", dtype={'CNAE Princ': str}, parse_dates=['Data de Referência'])
    ...
    df_placar_historico = pd.read_excel(f'W:/Otimização e Automatização/5. Projetos/22. Produção dos Pesquisadores/1. Dados/Histórico/historico_{tipo_sondagem}.xlsx', sheet_name = "Placar")
    ...
    df_telefone_historico = pd.read_excel(f'W:/Otimização e Automatização/5. Projetos/22. Produção dos Pesquisadores/1. Dados/Histórico/historico_{tipo_sondagem}.xlsx', sheet_name = "Telefone", parse_dates=['Data'])
    ...
    df_meta_historico = pd.read_excel(f'W:/Otimização e Automatização/5. Projetos/22. Produção dos Pesquisadores/1. Dados/Histórico/historico_{tipo_sondagem}.xlsx', sheet_name = "Meta", parse_dates=['Data de Referência'])
    ...
    if st.button('Clique aqui para atualizar o histórico'):
        with pd.ExcelWriter(f'W:/Otimização e Automatização/5. Projetos/22. Produção dos Pesquisadores/1. Dados/Histórico/historico_{tipo_sondagem}.xlsx') as writer:
            if tipo_sondagem != 'SEBRAE':
